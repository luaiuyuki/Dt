...
export default function BacSiPage() {
  ...
  const [form, setForm] = useState({ hoTen: '', chuyenKhoa: '', soDienThoai: '', email: '', luongCo: '', tyLeHoaHong: '', ngayBatDau: '' })
  const [editId, setEditId] = useState(null)
  const [saving, setSaving] = useState(false)
  const load = () => { setLoading(true); api.get('/bacSi').then((r) => { setList(r.data); setLoading(false) }) }
  useEffect(() => { load() }, [])
  const openAdd = () => { setForm({ hoTen: '', chuyenKhoa: '', soDienThoai: '', email: '', luongCo: '', tyLeHoaHong: '', ngayBatDau: '' }); setEditId(null); setShowModal(true) }
  const openEdit = (bs) => { setForm({ hoTen: bs.hoTen, chuyenKhoa: bs.chuyenKhoa || '', soDienThoai: bs.soDienThoai || '', email: bs.email || '', luongCo: bs.luongCo, tyLeHoaHong: bs.tyLeHoaHong, ngayBatDau: bs.ngayBatDau || '' }); setEditId(bs.id); setShowModal(true) }
  const handleSave = async (e) => {
    e.preventDefault()
    if (!form.hoTen.trim()) return alert('Vui Lòng Nhập Tên Bác Sĩ')
    setSaving(true)
    try {
  ...
  const handleDeactivate = async (id) => { if (!confirm('Ngừng Hoạt Động Bác Sĩ Này?')) return; await api.delete(`/bacSi/${id}`); load() }
      ...
      <div className="bg-white border-2 border-green-950 rounded overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-48 text-gray-500">
            ...
            <p className="text-sm font-medium">Chưa Có Bác Sĩ</p>
          </div>
        ) : (
              ...
              <tbody>
                {list.map((bs) => (
                  <tr key={bs.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4"><span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-800">{bs.tyLeHoaHong}%</span></td>
                    <td className="px-6 py-4"><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${bs.trangThai === 'hoatDong' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{bs.trangThai === 'hoatDong' ? 'Đang Làm' : 'Nghỉ'}</span></td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white border-2 border-green-950 rounded p-6 w-full max-w-lg shadow-2xl overflow-y-auto max-h-[90vh]">
            <h2 className="mb-6 text-xl font-semibold text-gray-900">{editId ? 'Cập Nhật Bác Sĩ' : 'Thêm Bác Sĩ Mới'}</h2>
            <form onSubmit={handleSave} className="grid grid-cols-2 gap-4">
              <div className="col-span-2"><label className={labelCls}>Họ & Tên *</label><input required className={inputCls} value={form.hoTen} onChange={f('hoTen')} placeholder="BS. Nguyễn Văn A"/></div>
              <div><label className={labelCls}>Chuyên Khoa *</label><input required className={inputCls} value={form.chuyenKhoa} onChange={f('chuyenKhoa')} placeholder="Phục Hình..."/></div>
              ...
              <div><label className={labelCls}>Email *</label><input required className={inputCls} type="email" value={form.email} onChange={f('email')} placeholder="bs@example.com"/></div>
              ...
              <div><label className={labelCls}>Hoa Hồng * (%)</label><input required className={inputCls} type="number" value={form.tyLeHoaHong} onChange={f('tyLeHoaHong')} placeholder="15"/></div>
